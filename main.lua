...
cmd:option('-corrupt',8,'corrupt input sequence of characters or not?')
-- data
...
-- CORRUPT!!!!!!
		...
		--		end
		--end
		--params:add(grad_params:mul(-optim_state.learningRate)) -- update params
		--if opt.hsm > 0 then
		--		hsm_params:add(hsm_grad_params:mul(-optim_state.learningRate))
		--end
		return {torch.exp(loss), loss}, grad_params
end
...
local optim_state = {learningRate = opt.learning_rate , alpha = 0.95}
...
for i = 1, iterations do
	iteration = i
		...
		local _, l = optim.rmsprop(feval, params, optim_state)
		train_loss, train_nll = l[1][1],l[1][2]
		...
		if i % loader.split_sizes[1] == 0 then
				...
				checkpoint.lr = optim_state.learningRate
		...
		if i % opt.print_every == 0 then
				print(string.format("%d/%d (epoch %.2f), train_loss = %6.4f, train_nll = %6.4f, time = %6.2f , lr = %6.4f", i, iterations, epoch, train_loss, train_nll, timer:time().real - time, optim_state.learningRate))
		end
